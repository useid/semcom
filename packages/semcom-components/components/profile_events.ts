...
import { LitElement, css, html, property, internalProperty, PropertyValues } from 'lit-element';
import type { Component } from '@digita-ai/semcom-core';

export default class ProfileEventComponent extends LitElement implements Component {
  ...
  constructor() {
    ...
    this.addEventListener('semcom-data-response', (event: CustomEvent & { detail: N3.Quad[] }) => this.processData(event.detail));

  }

  requestData(uri: string): void {

    this.dispatchEvent(new CustomEvent('semcom-data-request', {
      detail: { uri },
      bubbles: true,
      composed: true,
    }));
  ...
  update(changed: PropertyValues): void { if (changed.has('entry')) this.requestData(this.entry); }

  processData(quads: N3.Quad[]): void {
