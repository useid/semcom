...
describe('Server', () => {
    ...
    it('should return 200 on registered routes', (async () => {
        server.start({ controllers: [new ComponentControllerService(new ComponentMockService(new LoggerConsoleService(), [{ uri: 'test' }]), new LoggerConsoleService())], handlers: [] });
    ...
    it('should call a handlers canHandle', (async () => {
        const handler = new ServerHandlerContentNegotiationService(new LoggerConsoleService());
        handler.canHandle = mockListen;

        server.start({
            controllers: [new ComponentControllerService(new ComponentMockService(new LoggerConsoleService(), [{ uri: 'test' }]), new LoggerConsoleService())],
            handlers: [handler]
        });
    ...
    it('should return 404 on unknow routes', (async () => {
        server.start({ controllers: [new ComponentControllerService(new ComponentMockService(new LoggerConsoleService(), []), new LoggerConsoleService())], handlers: [] });
